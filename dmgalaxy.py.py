...
class GalaxySimulation(ShowBase):
    ...
    def init_grid(self, sparsity=0.1):
        ...
        # initialize every cell in the grid as an empty numpy array (no particles initially)
        self.grid = [[[[None, None] for _ in range(self.size)] for _ in range(self.size)] for _ in range(self.size)]
            ...
            for y in range(self.size):
                for z in range(self.size):
                    ...
                    if some_ranvar > 0.5:
                        ...
                        if not (np.isnan(current_position).any() or np.isinf(current_position).any()): # check if values are not NaN or Inf
                            instance = self.instance_root.attach_new_node("Instance")
                            instance.set_pos(Point3(*current_position)) # set the position using the array values
                            self.cube_model.instance_to(instance)
                            self.grid[x][y][z] = 1
                        else:
                            self.grid[x][y][z] = 0
